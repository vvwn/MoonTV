...
5. 仓库已内置 `wrangler.toml`（含 `nodejs_compat` 兼容性标志）。若你通过 Cloudflare 面板直接创建 Pages 项目，请在 **设置 -> 运行时** 确认兼容性标志包含 `nodejs_compat`
...
| TMDB_API_KEY                      | TMDB API Key（启用后用 TMDB 图片覆盖源站图片） | string                           | (空，表示禁用)                                                                                                             |
| TMDB_API_BASE_URL                 | TMDB API 地址（便于走代理或兼容 CF Node）      | url                              | https://api.themoviedb.org/3                                                                                               |
| TMDB_IMAGE_BASE_URL               | TMDB 图片地址前缀                              | url                              | https://image.tmdb.org/t/p/w500                                                                                            |
